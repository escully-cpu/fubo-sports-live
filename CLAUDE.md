...
## `~`-prefixed approximate dates were NEVER being verified (found 2026-09-11)

The user flagged: Dancing with the Stars, Survivor, and The Amazing Race were all sitting on **wrong** guessed dates (off by 1 to 15 days) despite the daily job running for weeks. Root cause was pure code, not a data problem: `parse_single_date()` — the function `verify_existing_dates()` and `verify_existing_times()` both use to decide whether an item's date is checkable — explicitly rejects any text containing `~` (`if any(c in text for c in ['~', ...]): return None`). Every approximate date in the calendar was therefore silently skipped by both verification passes, forever — they'd never even attempt an API call. Verified by hand with a raw TVMaze query: the correct date **and** time for all three shows (Survivor Sep 23 8PM, Amazing Race Sep 30 9:30PM, DWTS Sep 15 8PM) had been sitting in TVMaze the entire time. This was never a "TVMaze doesn't have it yet" problem like the Fall schedule lag above — it was a code path that structurally never ran.

Fixed with a new function, `resolve_approximate_dates()`, that specifically targets `~Mon DD` text (as opposed to `resolve_tbd_dates()`, which only handles literal `(TBD)`/`Fall (TBD)` text and was unaffected by this bug). For each match it looks up the real date via TVMaze (entertainment) or SportsDB (sports, also affected — same fix applies to both), and if found, replaces the guess and sets `data-time` in one shot, stripping the `~`. Runs as part of the normal daily `auto_update.py` pass, right alongside `resolve_tbd_dates`.

**The specific ask — "avoid that for events less than a week away, times and dates would be official by then"**: if no confirmed date/time turns up AND the guess is within 7 days of today, the run logs a loud `⚠️ UNCONFIRMED WITHIN 7 DAYS` warning (distinct log section, `Basso` notification sound instead of `Glass`) instead of silently leaving the guess in place. This won't fix the underlying data-availability lag (see the TVMaze section above — if a network genuinely hasn't confirmed something, no API call will invent it), but it guarantees the situation becomes visible with real time pressure attached, rather than a `~`-prefixed guess quietly riding all the way to its own air date unnoticed, which is what actually happened here.

If a similar "approximate date/time is structurally never checked" bug shows up elsewhere, the fix is the same shape: find what's actually calling `parse_single_date` (or any of its date-comparison siblings) and check whether it silently rejects and moves on for a whole category of real-world date formatting (`~`, ranges, "TBD", etc.) — don't assume "no corrections logged" means "nothing needed correcting."
